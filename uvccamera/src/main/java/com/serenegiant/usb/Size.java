...
public class Size implements Parcelable {
    ...
    private float getCurrentFrameRate() throws IllegalStateException {
        int n = fps != null ? fps.length : 0;
    ...
    private void updateFrameRate() {
        ...
        if (n > 0) {
            ...
            try {
                int min = Math.min(intervals[0], intervals[1]);
                int max = Math.max(intervals[0], intervals[1]);
                int step = intervals[2];
                if (step > 0) {
                    int m = 0;
