...
public class MediaSurfaceEncoder extends MediaEncoder implements IVideoEncoder {
    ...
    private static MediaCodecInfo selectVideoCodec() {
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            for (String type : types) {
                if (type.equalsIgnoreCase(MediaSurfaceEncoder.MIME_TYPE)) {
     ...
     */
    private static int selectColorFormat(final MediaCodecInfo codecInfo) {
        if (DEBUG) Log.i(TAG, "selectColorFormat: ");
        int result = 0;
        final MediaCodecInfo.CodecCapabilities caps;
        try {
            Thread.currentThread().setPriority(Thread.MAX_PRIORITY);
            caps = codecInfo.getCapabilitiesForType(MediaSurfaceEncoder.MIME_TYPE);
        } finally {
        ...
        if (result == 0)
            Log.e(TAG, "couldn't find a good color format for " + codecInfo.getName() + " / " + MediaSurfaceEncoder.MIME_TYPE);
        return result;
    }
