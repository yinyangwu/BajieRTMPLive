...
public class MediaVideoEncoder extends MediaEncoder implements IVideoEncoder {
    ...
    public MediaVideoEncoder(MediaMuxerWrapper muxer, int width, int height, MediaEncoderListener listener) {
    ...
    private static MediaCodecInfo selectVideoCodec() {
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            for (String type : types) {
                if (type.equalsIgnoreCase(MediaVideoEncoder.MIME_TYPE)) {
    ...
    private static int selectColorFormat(MediaCodecInfo codecInfo) {
        ...
        try {
            ...
            caps = codecInfo.getCapabilitiesForType(MediaVideoEncoder.MIME_TYPE);
        } finally {
        ...
        if (result == 0)
            Log.e(TAG, "couldn't find a good color format for " + codecInfo.getName() + " / " + MediaVideoEncoder.MIME_TYPE);
        return result;
    }
