...
public final class Handshake {
    ...
    public final void writeC1(OutputStream out) throws IOException {
        ...
        if (DEBUG) Log.d(TAG, "partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        if (DEBUG) Log.d(TAG, "writeC1(): Writing timestamp and Flash Player version");
        ...
        if (DEBUG) Log.d(TAG, "partAfterDigest(): size: " + partAfterDigest.length);
        ...
        if (DEBUG) Log.d(TAG, "writeC1(): writing C1 packet");
    ...
    public final void readS1(InputStream in) throws IOException {
        ...
        if (totalBytesRead != HANDSHAKE_SIZE) {
            ...
            if (DEBUG) Log.d(TAG, "readS1(): S1 total bytes read OK");
        }
    }
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            if (DEBUG) Log.d(TAG, "readS2(): S2 total bytes read OK");
        }
